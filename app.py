...
# --- A. 初期設定 ---
st.set_page_config(layout="wide", page_title="WT Online Ultimate v10")
...
def draw_tactical_map(grid, units, my_team_name):
    ...
    my_active_units = [u for u in units if u['team'] == my_team_name and u.get('is_active')]

    for u in units:
        if not u.get('is_active'): continue
        is_visible = (u['team'] == my_team_name)
        if not is_visible:
            for my_u in my_active_units:
                if math.sqrt((u['pos_x']-my_u['pos_x'])**2 + (u['pos_y']-my_u['pos_y'])**2) <= 5:
                    is_visible = True; break
        if is_visible:
            val = 6 if u['team'] == my_team_name else 7
            display_map[int(u['pos_x']), int(u['pos_y'])] = val
            hp_now, ap_now, z_now = int(u.get('hp', 0)), int(u.get('ap', 0)), int(u.get('pos_z', 0))
            label = f"{u['unit_name']}\nHP:{hp_now} Z:{z_now} AP:{ap_now}"
            label_bg = '#2ecc71' if u['team'] == my_team_name else '#e74c3c'
            ax.text(u['pos_y'], u['pos_x'] - 0.8, label, color='white', fontsize=8, 
                    fontweight='bold', ha='center', va='bottom', bbox=dict(facecolor=label_bg, alpha=0.9, boxstyle='round,pad=0.3'))
...
def draw_radar(units, my_team_name):
    fig, ax = plt.subplots(figsize=(4, 4), facecolor='#0d1117')
    ax.set_facecolor('#0d1117')
    for r in [3, 7, 11]:
        ax.add_artist(plt.Circle((7, 7), r, color='#1b5e20', fill=False, linestyle=':', alpha=0.5))
    for u in units:
        if u.get('is_active'):
            if u['team'] == my_team_name or u.get('selected_sub') != 'バッグワーム':
                color = '#2ecc71' if u['team'] == my_team_name else '#e74c3c'
...
def is_los_clear(u, e, grid):
    ...
    for i in range(1, steps):
        tx, ty = int(u['pos_x'] + (e['pos_x'] - u['pos_x']) * i / steps), int(u['pos_y'] + (e['pos_y'] - u['pos_y']) * i / steps)
        if grid[tx, ty] > max(int(u.get('pos_z', 0)), int(e.get('pos_z', 0))): return False
    return True

def resolve_turn(my_team_name, enemy_team_name, mode, grid):
    st.info("CPUの戦略を計算中...")
    ...
    logs = []
    my_pts, en_pts = int(session.get('my_points', 0)), int(session.get('enemy_points', 0))
    ...
    for u in units:
        ...
        # CPU思考ロジック
        if mode == "コンピューター（CPU）" and u['team'] == enemy_team_name:
            targets = [t for t in units if t['team'] != enemy_team_name and t['is_active']]
            if targets:
                target = min(targets, key=lambda t: abs(u['pos_x']-t['pos_x']) + abs(u['pos_y']-t['pos_y']))
                # 移動 (APを消費)
                if u['ap'] >= 5:
                    ...
                    u['ap'] -= 2
                # 攻撃準備 (メイン1を自動装備)
                u['selected_main'] = master['main1']
        
        # プレイヤー移動
        elif u.get('submitted_move'):
            move = u['submitted_move']
            ...
            if dist <= master['rng']:
                ...
                if is_los_clear(u, e, grid) or main_w in ['オルガノン', 'バイパー']:
                    ...
                    if e['hp'] <= 0:
                        ...
                        if u['team'] == enemy_team_name: en_pts += 1
                        else: my_pts += 1
    ...
    for u in units:
        supabase.table("unit_states").update({
            ...
            "ap": int(u['ap']), "trn": float(u.get('trn', 10)), "is_active": bool(u['is_active']), 
            "selected_main": u.get('selected_main', '-'), "submitted_move": None
        }).eq("unit_name", u['unit_name']).execute()
    supabase.table("game_session").update({"current_turn": int(session['current_turn']+1), "my_points": int(my_pts), "enemy_points": int(en_pts)}).eq("id", 1).execute()
    for l in logs: supabase.table("battle_logs").insert({"turn": int(session['current_turn']), "message": l}).execute()

# --- D. メイン UI ---
st.title("🛰️ WT Online: Ultimate v10 (CPU強化版)")
session = supabase.table("game_session").select("*").eq("id", 1).single().execute().data
...
with st.sidebar:
    ...
    st.pyplot(draw_radar(live_units, "カスタム" if st.session_state.get('entry') == "カスタム" else session.get('my_team', '操作チーム')))
    entry_mode = st.radio("編成", ["部隊プリセット", "カスタム編成"], key="entry")
    if entry_mode == "部隊プリセット":
        my_t = st.selectbox("自軍", df_master['team'].unique(), index=1)
        en_t = st.selectbox("敵軍", [t for t in df_master['team'].unique() if t != my_t])
    else:
        my_t, en_t, customs = "カスタム", "敵チーム", st.multiselect("メンバー(最大4名)", df_master['name'].unique())
    mode = st.radio("対戦", ["友人", "CPU"])
    if st.button("試合開始"):
        supabase.table("unit_states").delete().neq("id", 0).execute()
        supabase.table("battle_logs").delete().neq("id", 0).execute()
        sel = df_master[df_master['team'].isin([my_t, en_t])] if entry_mode=="部隊プリセット" else df_master[df_master['name'].isin(customs)]
        for _, row in sel.iterrows():
            supabase.table("unit_states").insert({"unit_name": row['name'], "team": row['team'] if entry_mode=="部隊プリセット" else "カスタム", "hp": 100, "ap": 20, "trn": float(row['trn']), "pos_x": random.randint(0, 14), "pos_y": random.randint(0, 14), "pos_z": 0, "is_active": True}).execute()
...
with col_map:
    ...
    st.pyplot(draw_tactical_map(st.session_state.grid, live_units, my_t))
    logs = supabase.table("battle_logs").select("*").order("id", desc=True).limit(5).execute().data
    for l in logs: st.caption(f"T{l['turn']}: {l['message']}")

with col_cmd:
    st.subheader("🎮 コマンド")
    my_active = [u for u in live_units if (u['team'] == my_t or u['team'] == "カスタム") and u.get('is_active')]
    for u in my_active:
        with st.expander(f"{u['unit_name']} (HP:{int(u['hp'])} AP:{u.get('ap')})"):
            m = df_master[df_master['name'] == u['unit_name']].iloc[0]
            cx1, cx2, cx3 = st.columns(3)
            nx, ny = int(cx1.number_input("X", 0, 14, u['pos_x'], key=f"x{u['unit_name']}")), int(cx2.number_input("Y", 0, 14, u['pos_y'], key=f"y{u['unit_name']}"))
            ...
            if st.button("確定", key=f"b{u['unit_name']}"):
                supabase.table("unit_states").update({"submitted_move": {"x": nx, "y": ny, "z": nz}, "selected_main": mt, "selected_sub": sub_t}).eq("unit_name", u['unit_name']).execute()
                st.success("予約完了")
    if st.button("🚨 全行動を解決"):
        resolve_turn(my_t, en_t, mode, st.session_state.grid)
        st.rerun()
