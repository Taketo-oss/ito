...
# --- A. 初期設定 ---
st.set_page_config(layout="wide", page_title="WT Online Ultimate v9")
supabase: Client = create_client(st.secrets["SUPABASE_URL"], st.secrets["SUPABASE_KEY"])

GRID_SIZE = 15
df_master = pd.read_csv("units.csv")

# --- B. 描画エンジン (レーダー・メインマップ) ---
...
def resolve_turn(my_team_name, enemy_team_name, mode, grid):
    ...
    for u in units:
        ...
        elif mode == "コンピューター（CPU）" and u['team'] == enemy_team_name:
            targets = [t for t in units if t['team'] == my_team_name and t['is_active']]
            if targets:
            ...
            if dist <= master['rng'] and main_w != '-':
                ...
                if is_los_clear(u, e, grid) or main_w in ['オルガノン', 'バイパー']:
                    ...
                    if e['hp'] <= 0:
                        ...
                        if u['team'] == my_team_name: my_pts += 1
                        else: en_pts += 1
...
st.title("🛰️ WT Online: Ultimate v9")
...
with st.sidebar:
    ...
    if entry_mode == "部隊プリセット":
        my_t_name = st.selectbox("自分の部隊", df_master['team'].unique(), index=1)
        en_t_name = st.selectbox("敵部隊", [t for t in df_master['team'].unique() if t != my_t_name])
    else:
        my_t_name = "カスタム"; en_t_name = "敵チーム"
    ...
    if st.button("試合開始（初期化）"):
        ...
        selected = df_master[df_master['team'].isin([my_t_name, en_t_name])] if entry_mode=="部隊プリセット" else df_master[df_master['name'].isin(customs)]
...
with col_map:
    ...
    st.pyplot(draw_tactical_map(st.session_state.grid, live_units, "カスタム" if entry_mode=="カスタム" else my_t_name))
...
with col_cmd:
    st.subheader("🎮 コマンド入力")
    my_active = [u for u in live_units if (u['team'] == my_t_name or u['team'] == "カスタム") and u.get('is_active')]
    for u in my_active:
        with st.expander(f"{u['unit_name']} (HP:{int(u['hp'])} AP:{u.get('ap')})"):
            ...
            # 【重要修正】 変数名を st から sub_t に変更して Attributeエラーを防止
            ...
            if st.button("確定保存", key=f"b{u['unit_name']}"):
                supabase.table("unit_states").update({
                    "submitted_move": {"x": nx, "y": ny, "z": nz}, "selected_main": mt, "selected_sub": sub_t
                }).eq("unit_name", u['unit_name']).execute()
                st.success("予約完了")

    if st.button("🚨 全行動を解決する"):
        resolve_turn(my_t_name, en_t_name, mode, st.session_state.grid)
        st.rerun()
